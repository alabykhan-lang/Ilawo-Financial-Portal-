...
async function loadPortalData(client: AnyClient, user: User): Promise<PortalData> {
  ...
  const [classes, sessions, terms, categories, categoryClasses, students, charges, payments, corrections, handovers, handoverItems] = await Promise.all([
    ...
    can("view_students") || can("record_student_payments") || can("view_school_reports") ? fetchTable(client, "students", "id,full_name,class_id,arm,status,academic_session_id") : Promise.resolve([]),
  ...
  const decoratedPayments = portalPayments.map((payment) => ({
    ...
    student: studentMap.get(payment.student_id) ? { full_name: studentMap.get(payment.student_id)!.full_name } : undefined,
...
function PaymentsView({ data, can, client, onRefresh, onToast }: { data: PortalData; can: (permission: PermissionKey) => boolean; client: AnyClient; onRefresh: () => Promise<void>; onToast: (toast: NonNullable<Toast>) => void }) {
  ...
  const studentsInClass = data.students.filter((student) => !classId || student.class_id === classId).filter((student) => `${student.full_name}`.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="content-wrap">
      ...
      {mode === "single" && can("record_student_payments") && <section className="panel form-panel"><div className="panel-heading"><div><span className="section-kicker">QUICK ENTRY</span><h2>Record one payment</h2></div><p className="muted">Review before confirming. It cannot be edited afterwards.</p></div><form className="stack-form" onSubmit={submitSingle}><div className="form-grid"><SelectField label="Class" required value={classId} onChange={(value) => { setClassId(value); setStudentId(""); setCategoryId(""); }}><option value="">Choose class</option>{data.classes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</SelectField><TextField label="Search student" value={search} onChange={setSearch} placeholder="Student name" /></div><SelectField label="Student" required value={studentId} onChange={setStudentId}><option value="">Choose student</option>{studentsInClass.slice(0, 100).map((student) => <option key={student.id} value={student.id}>{student.full_name}</option>)}</SelectField><div className="form-grid"><SelectField label="Financial category" required value={categoryId} onChange={setCategoryId}><option value="">Choose category</option>{categoriesForClass.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</SelectField><TextField label="Amount paid (₦)" required type="number" min="0.01" step="0.01" value={amount} onChange={setAmount} placeholder="0.00" /></div><div className="form-grid"><SelectField label="Academic session" required value={sessionId} onChange={(value) => { setSessionId(value); setTermId(""); }}><option value="">Choose session</option>{data.sessions.map((session) => <option key={session.id} value={session.id}>{session.name}</option>)}</SelectField><SelectField label="Term (where applicable)" value={termId} onChange={setTermId}><option value="">Not term-specific</option>{sessionTerms.map((term) => <option key={term.id} value={term.id}>{term.name}</option>)}</SelectField></div><div className="form-grid"><TextField label="Payment date" required type="date" value={paymentDate} onChange={setPaymentDate} /><TextField label="Optional note" value={note} onChange={setNote} placeholder="Receipt note" /></div>{studentId && categoryId && <div className="review-box"><span className="section-kicker">CHECK BEFORE SAVING</span><strong>{data.students.find((student) => student.id === studentId)?.full_name}</strong><div><span>{data.categories.find((category) => category.id === categoryId)?.name}</span><b>{naira(amount)}</b></div></div>}<button className="button primary full" disabled={busy || !studentId || !categoryId}>{busy ? "Saving…" : "Confirm & lock payment"}</button></form></section>}
      {mode === "batch" && can("record_student_payments") && <section className="panel form-panel"><div className="panel-heading"><div><span className="section-kicker">QUICK CLASS ENTRY</span><h2>Record several students</h2></div><p className="muted">Only rows with an amount will be created.</p></div><form className="stack-form" onSubmit={submitBatch}><div className="form-grid"><SelectField label="Class" required value={classId} onChange={(value) => { setClassId(value); setBatchAmounts({}); setBatchReview(false); }}><option value="">Choose class</option>{data.classes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</SelectField><SelectField label="Financial category" required value={categoryId} onChange={(value) => { setCategoryId(value); setBatchReview(false); }}><option value="">Choose category</option>{categoriesForClass.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</SelectField></div><div className="form-grid"><SelectField label="Academic session" required value={sessionId} onChange={(value) => { setSessionId(value); setTermId(""); setBatchReview(false); }}><option value="">Choose session</option>{data.sessions.map((session) => <option key={session.id} value={session.id}>{session.name}</option>)}</SelectField><SelectField label="Term (where applicable)" value={termId} onChange={(value) => { setTermId(value); setBatchReview(false); }}><option value="">Not term-specific</option>{sessionTerms.map((term) => <option key={term.id} value={term.id}>{term.name}</option>)}</SelectField></div><div className="form-grid"><TextField label="Payment date" required type="date" value={paymentDate} onChange={(value) => { setPaymentDate(value); setBatchReview(false); }} /><TextField label="Shared note" value={note} onChange={(value) => { setNote(value); setBatchReview(false); }} placeholder="Optional note for all" /></div>{classId ? <div className="batch-list">{data.students.filter((student) => student.class_id === classId && student.status === "active").map((student) => <div className="batch-row" key={student.id}><div><strong>{student.full_name}</strong><span>{student.full_name}</span></div><input aria-label={`Amount for ${student.full_name}`} type="number" min="0" step="0.01" placeholder="₦ 0" value={batchAmounts[student.id] || ""} onChange={(e) => { setBatchAmounts({ ...batchAmounts, [student.id]: e.target.value }); setBatchReview(false); }} /></div>)}</div> : <p className="muted">Choose a class to see its students.</p>}{batchReview && <div className="review-box"><span className="section-kicker">REVIEW BATCH</span><strong>{Object.values(batchAmounts).filter((value) => numberValue(value) > 0).length} payment records · {naira(Object.values(batchAmounts).reduce((sum, value) => sum + numberValue(value), 0))}</strong><p className="muted">Confirming will create each row as a locked transaction.</p></div>}<button className="button primary full" disabled={busy || !classId || !categoryId}>{busy ? "Saving batch…" : batchReview ? "Confirm & lock batch" : "Review batch entries"}</button>{batchReview && <button type="button" className="button ghost full" onClick={() => setBatchReview(false)}>Back to edit</button>}</form></section>}
      {mode === "history" && <PaymentHistory data={data} can={can} onRequestCorrection={setSelectedCorrection} onReviewCorrection={reviewCorrection} busy={busy} />}
      {mode !== "history" && data.corrections.some((request) => request.status === "pending") && (data.profile.role === "principal" || can("view_school_reports")) && <CorrectionQueue data={data} onReview={reviewCorrection} busy={busy} />}
      {selectedCorrection && <Modal title="Request a correction" onClose={() => setSelectedCorrection(null)}><form className="stack-form" onSubmit={requestCorrection}><div className="locked-summary"><strong>{selectedCorrection.student?.full_name}</strong><span>{selectedCorrection.reference_no} · {selectedCorrection.category?.name} · {naira(selectedCorrection.amount_paid)}</span></div><SelectField label="Request type" value={correctionAction} onChange={(value) => setCorrectionAction(value as "reverse" | "correct")}><option value="reverse">Reverse this payment</option><option value="correct">Correct the amount</option></SelectField>{correctionAction === "correct" && <TextField label="Correct amount (₦)" required type="number" min="0.01" step="0.01" value={correctionAmount} onChange={setCorrectionAmount} placeholder="0.00" />}<label className="field-label">Reason<textarea required minLength={5} value={correctionReason} onChange={(e) => setCorrectionReason(e.target.value)} placeholder="Explain what needs to be corrected" /></label><p className="warning-line">The original transaction stays unchanged. The Principal must approve this request.</p><button className="button primary full" disabled={busy}>{busy ? "Sending…" : "Send correction request"}</button></form></Modal>}
    </div>
  );
}

function PaymentHistory({ data, can, onRequestCorrection, onReviewCorrection, busy }: { data: PortalData; can: (permission: PermissionKey) => boolean; onRequestCorrection: (payment: Payment) => void; onReviewCorrection: (request: CorrectionRequest, approve: boolean) => Promise<void>; busy: boolean }) {
  const [search, setSearch] = useState("");
  const filtered = data.payments.filter((payment) => `${payment.student?.full_name || ""} ${payment.reference_no} ${payment.category?.name || ""}`.toLowerCase().includes(search.toLowerCase()));
  return <section className="panel"><div className="panel-heading"><div><span className="section-kicker">READ-ONLY LEDGER</span><h2>Payment history</h2></div><input className="compact-search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name or reference" /></div>{filtered.length ? <div className="table-scroll"><table><thead><tr><th>Student</th><th>Category</th><th>Amount</th><th>Collected by</th><th>When</th><th>Reference</th><th /></tr></thead><tbody>{filtered.map((payment) => <tr key={payment.id}><td><strong>{payment.student?.full_name || "—"}</strong></td><td>{payment.category?.name || "—"}</td><td><strong>{naira(payment.amount_paid)}</strong></td><td>{payment.collector?.full_name || (payment.collector_id === data.profile.id ? data.profile.full_name : "Staff")}</td><td>{shortDate(payment.payment_date)}<small>{dateTime(payment.collected_at)}</small></td><td><code>{payment.reference_no}</code></td><td>{can("record_student_payments") && <button className="small-button" onClick={() => onRequestCorrection(payment)}>Correction</button>}</td></tr>)}</tbody></table></div> : <EmptyState title="No payments yet" text="Locked payment records will appear here after the first entry." />}</section>;
}

function CorrectionQueue({ data, onReview, busy }: { data: PortalData; onReview: (request: CorrectionRequest, approve: boolean) => Promise<void>; busy: boolean }) {
  return <section className="panel"><div className="panel-heading"><div><span className="section-kicker">PRINCIPAL REVIEW</span><h2>Correction requests</h2></div><Badge tone="warning">{data.corrections.filter((request) => request.status === "pending").length} pending</Badge></div><div className="request-list">{data.corrections.filter((request) => request.status === "pending").map((request) => <div className="request-row" key={request.id}><div><strong>{request.original_payment?.student?.full_name || "Payment"}</strong><span>{request.action === "reverse" ? "Request reversal" : `Correct to ${naira(request.requested_amount)}`} · {request.original_payment?.reference_no}</span><p>{request.reason}</p></div><div className="row-actions"><button className="small-button success" disabled={busy} onClick={() => void onReview(request, true)}>Approve</button><button className="small-button danger" disabled={busy} onClick={() => void onReview(request, false)}>Reject</button></div></div>)}</div></section>;
}

function HandoverView({ data, can, client, onRefresh, onToast }: { data: PortalData; can: (permission: PermissionKey) => boolean; client: AnyClient; onRefresh: () => Promise<void>; onToast: (toast: NonNullable<Toast>) => void }) {
  const [selected, setSelected] = useState<string[]>([]);
  const [decisionNote, setDecisionNote] = useState("");
  const [busy, setBusy] = useState(false);
  const confirmedIds = useMemo(() => new Set(data.handoverItems.filter((item) => data.handovers.find((handover) => handover.id === item.handover_id)?.status === "confirmed").map((item) => item.payment_id)), [data.handoverItems, data.handovers]);
  const pendingIds = useMemo(() => new Set(data.handoverItems.filter((item) => data.handovers.find((handover) => handover.id === item.handover_id)?.status === "pending").map((item) => item.payment_id)), [data.handoverItems, data.handovers]);
  const custodyPayments = data.payments.filter((payment) => payment.collector_id === data.profile.id && !confirmedIds.has(payment.id) && !pendingIds.has(payment.id));
  const selectedTotal = selected.reduce((sum, id) => sum + numberValue(data.payments.find((payment) => payment.id === id)?.amount_paid), 0);
  const pending = data.handovers.filter((handover) => handover.status === "pending");

  async function createHandover() {
    if (!selected.length) return;
    setBusy(true);
    const { error } = await client.rpc("create_handover", { p_payment_ids: selected });
    setBusy(false);
    if (error) { onToast({ type: "error", message: error.message }); return; }
    setSelected([]);
    onToast({ type: "success", message: "Handover submitted for Principal confirmation." });
    await onRefresh();
  }
...
function StudentsView({ data, can, client, onRefresh, onToast }: { data: PortalData; can: (permission: PermissionKey) => boolean; client: AnyClient; onRefresh: () => Promise<void>; onToast: (toast: NonNullable<Toast>) => void }) {
  ...
  const [busy, setBusy] = useState(false);
  const students = data.students.filter((student) => (!classId || student.class_id === classId) && `${student.full_name}`.toLowerCase().includes(search.toLowerCase()));
  ...
  async function importStudents() {
    ...
    const rows = importRows.map((row) => ({ full_name: row.full_name, class_id: classMap.get((row.class || "").toLowerCase()), arm: row.arm || null, status: ["active", "inactive", "graduated", "withdrawn"].includes(row.status) ? row.status : "active", academic_session_id: sessionMapByName.get((row.academic_session || "").toLowerCase()) })).filter((row) => row.full_name && row.class_id && row.academic_session_id);
  ...
  function downloadTemplate() {
    const csv = ["full_name,class,arm,status,academic_session", ""].join("\n");
